...
var envFactory = function($http) {
  ...
  return {
    ...
    isMissingEnvJS: function () {
      return missingEnvJS
    },
    KAFKA_LENSES_URL: function () {
      return selectedCluster.KAFKA_LENSES_URL.trim();
    },
    DEBUG_LOGS_ENABLED: function () {
      return selectedCluster.DEBUG_LOGS_ENABLED.trim();
    }
  };
  ...
  function setCluster(clusterName) {
    ...
    if (angular.isUndefined(clusterName)) {
      ...
      var filteredArray = allClusters.filter(function (el) {
